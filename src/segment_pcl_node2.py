...
class pcl_node_handler:
  def __init__(self, roi_topic, depth_image_topic, cam_info_topic):
      ...
      self.pub = rospy.Publisher('/thresholded_depths',Image,queue_size=1)
  ...
  def publish_pcl(self):
    ...
    if self.has_roi == True:
      for box in roi.bounding_boxes:
        if box.Class in self.class_list:   # check if object is in class listdepth_frame
          ...
          contours = np.array([[box.xmin,box.ymin], [box.xmin,box.ymax], [box.xmax,box.ymax], [box.xmax,box.ymin]])
          # mask = np.ones(shape=depth_frame_roi.shape[0:2], dtype="bool")
          # rr, cc = skimage.draw.rectangle(start=(box.ymax,box.xmax), end=(box.ymin,box.xmin))
          # mask[rr, cc] = False
          
          # cv2.fillPoly(depth_frame_roi, pts = [contours], color =(255,255,255))
          th = threshold_otsu(depth_frame_roi[box.ymin:box.ymax, box.xmin:box.xmax], nbins=65635)
          depth_frame_roi[depth_frame>th] = 0
          # depth_frame_roi[mask] = 0
          # depth_frame_roi = depth_frame_roi/1000.0  # convert to m as float 32
          out_msg = cvb.cv2_to_imgmsg(depth_frame_roi, encoding="passthrough")
